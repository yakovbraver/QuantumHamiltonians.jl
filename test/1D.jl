@testset "Test dense 1D 1-component diagonalisation" begin
    ...
    ### Periodic
    xh = XSpaceHamiltonian{:dense}([xlimits], 𝑈; isperiodic=true, 𝑈_iseven=true, M=10, δ=(√0.5f0)) # `√` because `δ` is the coefficient of ∂ₓ, not Δ
    ...
    ### Nonperiodic
    xh = XSpaceHamiltonian{:dense}([xlimits], 𝑈; isperiodic=false, M=15, δ=(√0.5f0)) # `√` because `δ` is the coefficient of ∂ₓ, not Δ
    @test xh isa XSpaceHamiltonians.DenseHamiltonian{Float32,Float32,Float32,2,3}

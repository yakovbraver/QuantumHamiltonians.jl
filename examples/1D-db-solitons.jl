...
@time xs, sol = find_stationary(xh_db, [tanh, sech], g, μs; abstol=1e-12, show_trace=Val(true))
ψ_db = sol.u
E, μs, ηs = get_Eμη(xh_db, ψ_db, g; v_is_pspace=false)
plot_comps(xs, ψ_db)
...
vals, vecs = bdg_spectrum(xh_db, ψ_db, g, μs) # also works with nev=10 and/or `storage=:sparse`. `storage=:lazy` fails.
...
vals, vecs = bdg_spectrum(xh_db, Ψ₀_sampled, g, μs)
...
figs = [heatmap(xs, sol.t, abs2.(U[:, :, c])', c=CMAP, xlabel="x", ylabel="t") for c in 1:xh_db.nc]
plot(figs..., layout=(xh_db.nc, 1))
...
function scan_gs(gs, sol, qs=nothing; whichg=12, nev=0, abstol=1e-8, bdg_verbose=false, xspace=true)
     D = length(xh.xlims)
     B = (2M + 1)^D # block size
     nsaves = nev == 0 ? 2B*xh.nc : nev # number of eigenvalues and eigenvectors to store: if `nev` is zero (or not passed), then store all
     ...
     @showprogress for (i, gi) in enumerate(gs)
          ...
          xs, sol = find_stationary(xh, [sol.u[1:end÷2], sol.u[end÷2+1:end]], g, μs; abstol)
          Int(sol.retcode) != 1 && println("\n i = $i: sol.retcode = $(sol.retcode)\n maximum(resid) = $(maximum(sol.resid))") # print retcode if unsuccessful
          if xspace
               if isnothing(qs)
                    vals, _ = bdg_spectrum(xh, sol.u, g, μs; nev, verbose=bdg_verbose)
                    data[:, i] = vals[1:nsaves]
               else
                    vals, _ = bdg_spectrum(xh, sol.u, g, μs, qs; nev, verbose=bdg_verbose)
                    data[:, i] = reshape(vals, :)
               end
          else
               if isnothing(qs)
...
# nlsolve
@time xs, sol = find_stationary(xh, Ψ₀, g, μs, abstol=1e-11, show_trace=Val(true)) 
ψ_lattice = sol.u
get_Eμη(xh, ψ_lattice, g, v_is_pspace=false)
...
smallindx = findall(x -> abs(x) < 1e-5, vals)
vals[smallindx]
...
data = scan_gs(g12s, sol; nev=100, abstol=1e-11, xspace=false) # in p-space, Arnoldi works fine, so can use e.g. nev=100
...
@time xs, sol = find_stationary(xh, Ψ₀, g, μs; abstol=1e-10, show_trace=Val(true))
ψ_lattice = sol.u
plot_comps(xs, ψ_lattice)
get_Eμη(xh, ψ_lattice, g, v_is_pspace=false)
...
figs = [heatmap(xs, sol.t, abs2.(U[:, :, c])', c=CMAP, xlabel="x", ylabel="t") for c in 1:xh.nc]
plot(figs..., layout=(xh.nc, 1))
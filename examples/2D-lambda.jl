...
# Here we have no 𝐴, so Hamiltonian xh.H will be real. Eigenfunctions also since basis is real in nonperiodic case. `𝐻_iseven` is not used in the nonperiodic case
xh = XSpaceHamiltonian{:dense}(xlimits, ylimits; isperiodic=false, M, 𝐻=[𝑈;;]);

# Or we can solve periodic case. Potential is even, so we set `𝐻_iseven`, yielding a real Hamiltonian
xh = XSpaceHamiltonian{:dense}(xlimits, ylimits; isperiodic=true, M, 𝐻=[𝑈;;], 𝐻_iseven=[true;;])
...
@time xh = XSpaceHamiltonian{:dense}(xlimits, ylimits; isperiodic=false, M, 𝐻=[𝑈;;], 𝐴_x, 𝐴_y);
...
@time xh = XSpaceHamiltonian{:dense}(xlimits, ylimits; isperiodic=true, M, 𝐻=[𝑈;;], 𝐻_iseven=[true;;], 𝐴_x, 𝐴_y);
...
@time xh = XSpaceHamiltonian{:dense}(xlimits, ylimits; isperiodic=true, M, 𝐻=[𝑈;;], 𝐻_iseven=[true;;], 𝐴_x, 𝐴_y); # 56 s construct + 10 s diagonalise
